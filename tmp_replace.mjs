...
const filePath = "src/components/Hero.astro";
...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('// Downgrade active region to "visited" orange when leaving, then highlight new as focus')) {
        startIdx = i;
        break;
    }
}
...
for (let i = startIdx; i < lines.length; i++) {
    if (lines[i].includes('function animate() {') || (i > startIdx && lines[i].includes('function animate'))) {
        endIdx = i;
        break;
    }
}
...
const replacement = `    // Downgrade active region to "visited" orange when leaving, then highlight new as focus
    const focusRegion = (
      names: string[],
      prevNames: string[],
...
console.log("Replaced", endIdx - startIdx, "lines successfully with Node.js");
